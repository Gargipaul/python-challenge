...
with open(budget_data, 'w') as writefile:
    writefile.writelines('Financial Analysis\n')
    writefile.writelines('----------------------------' + '\n')
    writefile.writelines(f"Total months: {str(total_months)}" + '\n')
    writefile.writelines(f"Total: ${str(total_revenue)}" + '\n')
    writefile.writelines(f"Average Change: ${str(round(avg_change,2))}" + '\n')
    writefile.writelines(f"Greatest Increase in Profits: {Greatest_date} (${str(Greatest_increase)})" + '\n')
    writefile.writelines(f"Greatest Decrease in Profits: {loss_date} (${str(Greatest_decrease)})" + ')')
